...
<?php
    ...
    if ( ! is_page() ) : //固定ページを表示していない場合 ?>
        <div class="d-inline entry-meta">
            <time class="mr-2 post-date" datetime="<?php the_date('c'); ?>">公開日:<?php echo get_the_date(); ?></time>
            <?php if ( get_the_date() != get_the_modified_date() ) : //公開日と最終更新日が一致しない場合のみ最終更新日を表示?>
                <time class="mr-2 modified-date" datetime="<?php the_modified_time('c'); ?>">最終更新日:<?php echo the_modified_date(); ?></time>
            <?php endif; ?>
